module.exports = function (nit, cron, Self)
{
    return (Self = cron.defineItem ("Range"))
        ...
        .method ("parseValue", function (str)
        {
            ...
            if (!nit.is.undef (v = cls.aliases[nit.trim (str).toUpperCase ()]))
            {
                return v;
            }
            else
            {
                self.throw ("error.invalid_value", { value: str });
            }
        })
        ...
        .onGetStepsToNextOccurrence (function (date)
        {
            ...
            let values = self.values.reduce ((a, v) =>
            {
                a.push (v);

                let rv = self.field.getRealValueForDate (v, date);
