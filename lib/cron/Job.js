module.exports = function (nit, cron, Self)
{
    ...
    return (Self = nit.defineClass ("cron.Job"))
        ...
        .field ("[timezone]", "nit.Date.Timezone", "The time zone.", function ()
        {
            return new nit.Date.Timezone (nit.timezone ());
        })
        ...
        .property ("logger", "any",
        {
            constraints: nit.new ("constraints.Plugin", "plugins.Logger")
        })
        ...
        .method ("run", async function ()
        {
            ...
            try
            {
                ...
                child.on ("close", code =>
                {
                    ...
                    {
                        self.logger?.info?. (Buffer.concat (stdoutBuffers).toString ());
                    }

                    if (stderrBuffers.length)
                    {
                        self.logger?.error?. (Buffer.concat (stderrBuffers).toString ());
                    }
