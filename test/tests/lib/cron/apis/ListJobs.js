test.api ("cron.apis.ListJobs")
    ...
    .before (s => s.scheduler = s.Scheduler.get (s.context.server))
    .before (s => s.scheduler.jobMap[9] = nit.new ("cron.Job", 9, "0 0 * * *", "nit test", "America/Indianapolis"))
    .before (s => s.scheduler.jobMap[10] = nit.new ("cron.Job", 1, "0 0 * * *", "nit test", "America/Indianapolis"))
    .before (s => s.scheduler.jobMap[9].next (nit.Date ("2023-09-18 15:00", "America/Indianapolis")))
    .before (s => s.scheduler.jobMap[10].next (nit.Date ("2023-09-18 15:00", "America/Indianapolis")))
